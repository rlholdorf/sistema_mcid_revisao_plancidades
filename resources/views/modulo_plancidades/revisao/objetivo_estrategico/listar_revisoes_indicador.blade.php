                    ...
                    <tbody>
                        @foreach ($revisoes as $revisao)
                            <tr class="conteudoTabela">
                                ...
                                <td>{{ $revisao->txt_denominacao_indicador}}</td>
                                <td class="text-center">{{($revisao->dsc_periodo_revisao != null ) ? ($revisao->dsc_periodo_revisao)."/".($revisao->num_ano_periodo_revisao) : 'Não monitorado'}}</td>
                                ...
                                <td class="acao"><a class="br-button circle primary small"
                                    href='{{ route("plancidades.revisaos.objetivoEstrategico.show", ['indicadorId' => $revisao->revisao_indicador_id]) }}'><i
                                ...
                                <td class="acao" {{(($revisao->situacao_revisao_id == '3' ) || ($revisao->situacao_revisao_id == '5' ) || ($revisao->situacao_revisao_id == '6')) ? 'disabled' : '' }}><a class="br-button circle primary small"
                                    href='{{ route("plancidades.revisaos.objetivoEstrategico.editar", ['indicadorId' => $revisao->revisao_indicador_id]) }}'><i
